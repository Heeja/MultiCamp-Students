...
1. 정적 쿼리를 사용한다. (= 구조화된 쿼리 실행 = 파라미터화된 쿼리 실행 = 입력값에 따라 쿼리문의 구조가 바뀌지 않도록 한다.)
   = **PreparedStatement** 객체를 이용해서 쿼리를 실행한다.
2. ORM Framework를 사용하는 경우, 외부 입력값을 쿼리맵(XML)에 바인딩 할 때 반드시 '#'기호를 이용한다.
3. 오류 메시지에 시스템 정보가 노출되지 않도록 한다.
   1. Error를 이용한 공격을 예방하기 위함이다.
